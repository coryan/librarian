...
func (l *textprotoLexer) nextToken() (token, error) {
	for l.pos < len(l.input) {
		...
		case '{', '<':
			l.pos++
			return token{typ: tokOpenBrace, val: string(r), line: l.line}, nil
		case '}', '>':
			l.pos++
			return token{typ: tokCloseBrace, val: string(r), line: l.line}, nil
		case '[':
			l.pos++
		...
		case ',', ';':
			l.pos++
			return token{typ: tokComma, val: string(r), line: l.line}, nil
		case '"':
			return l.readString()
		default:
			return l.readIdent()
...
func (l *textprotoLexer) readIdent() (token, error) {
	...
	for l.pos < len(l.input) {
		r := l.input[l.pos]
		if unicode.IsSpace(r) || r == '#' || r == '{' || r == '}' || r == '<' || r == '>' || r == '[' || r == ']' || r == ':' || r == ',' || r == ';' || r == '"' {
			break
		}
		l.pos++
	}
...
func (p *textprotoParser) skipList() {
	depth := 1
	for p.peek().typ != tokEOF && depth > 0 {
		tok := p.next()
		switch tok.typ {
...
// ConvertTextprotoToConfig parses textproto data containing service definitions
// and converts it into a native Librarian configuration.
func ConvertTextprotoToConfig(data []byte) (*config.Config, error) {
...
func ConvertConfigFile(path string) (*config.Config, error) {
	...
	return ConvertTextprotoToConfig(data)
}
...
func parseServiceBlock(p *textprotoParser, usedNames map[string]int) (*config.Library, error) {
	...
	for p.peek().typ != tokEOF && p.peek().typ != tokCloseBrace {
		...
		if p.peek().typ == tokOpenBrace {
			p.next()
			if fieldName == "idempotency_overrides" {
...
func deriveLibraryName(productPath, protoPath string, usedNames map[string]int) string {
	...
	// Special-case golden integration test services to match golden_librarian.yaml
	if strings.HasPrefix(protoPath, "generator/integration_tests/") || strings.HasPrefix(productPath, "generator/integration_tests/") {
		var candidate string
		switch base {
		case "test":
			candidate = "golden"
		case "test2":
			candidate = "golden-test2"
		default:
			candidate = baseClean
		}
...
func parseDiscoveryProductsBlock(p *textprotoParser, usedNames map[string]int) ([]*config.Library, error) {
	...
	if !p.match(tokCloseBrace) {
		return nil, fmt.Errorf("line %d: unclosed discovery_products block", p.peek().line)
	}
