...
func TestAssuredWorkloads_PilotParity(t *testing.T) {
	cppRepo := locateGoogleCloudCpp(t)
	if cppRepo == "" {
		t.Skip("skipping integration test: google-cloud-cpp repo not found")
	}
	...
	outDir := filepath.Join(tmpDir, "google", "cloud", "assuredworkloads", "v1")
	lib := &config.Library{
		Name:   "google-cloud-assuredworkloads-v1",
		Output: outDir,
		APIs: []*config.API{
			{Path: "google/cloud/assuredworkloads/v1/assuredworkloads.proto"},
		},
		Cpp: &config.CppLibrary{
	...
	refV1Dir := filepath.Join(cppRepo, "google", "cloud", "assuredworkloads", "v1")
	for _, relPath := range generatedVersionedFiles {
		t.Run("v1/"+relPath, func(t *testing.T) {
			gotBytes, err := os.ReadFile(filepath.Join(outDir, relPath))
			if err != nil {
	...
	// Verify forwarding headers
	fwdDir := filepath.Join(tmpDir, "google", "cloud", "assuredworkloads")
	refFwdDir := filepath.Join(cppRepo, "google", "cloud", "assuredworkloads")
	...
	expectedForwarding := []string{
		...
		filepath.Join("mocks", "mock_assured_workloads_connection.h"),
	}
...
func TestComputeAddresses_PilotParity(t *testing.T) {
	cppRepo := locateGoogleCloudCpp(t)
	if cppRepo == "" {
		t.Skip("skipping integration test: google-cloud-cpp repo not found")
	}
	...
	falseVal := false
	outDir := filepath.Join(tmpDir, "google", "cloud", "compute", "addresses", "v1")
	lib := &config.Library{
		Name:   "google-cloud-compute-addresses-v1",
		Roots:  []string{"googleapis", filepath.Join(cppRepo, "protos")},
		Output: outDir,
		APIs: []*config.API{
			{Path: "google/cloud/compute/addresses/v1/addresses.proto"},
		},
		Cpp: &config.CppLibrary{
	...
	refV1Dir := filepath.Join(cppRepo, "google", "cloud", "compute", "addresses", "v1")
	for _, relPath := range generatedVersionedFiles {
		t.Run("v1/"+relPath, func(t *testing.T) {
			gotBytes, err := os.ReadFile(filepath.Join(outDir, relPath))
			if err != nil {
