	...
	t.Run("comments.proto symbols", func(t *testing.T) {
		...
		for _, test := range []struct {
			...
			t.Run(test.symbol, func(t *testing.T) {
				...
				if !ok {
					t.Fatalf("missing definition location for %q", test.symbol)
				}
...
func TestExtractDefinitionLocations_ExtensionsAndEdgeCases(t *testing.T) {
	...
	t.Run("nil and empty edge cases", func(t *testing.T) {
		...
		if diff := cmp.Diff(0, model.DefinitionLocationCount()); diff != "" {
			t.Errorf("mismatch (-want +got):\n%s", diff)
		}
	})
	...
	t.Run("oneof definition location", func(t *testing.T) {
		fileDesc := &descriptorpb.FileDescriptorProto{
			Name:    new("oneof_test.proto"),
			Package: new("test.oneof"),
			MessageType: []*descriptorpb.DescriptorProto{
				{
					Name: new("Msg"),
			...
			SourceCodeInfo: &descriptorpb.SourceCodeInfo{
				Location: []*descriptorpb.SourceCodeInfo_Location{
					{
						Path: []int32{fileDescriptorMessageType, 0, messageDescriptorOneOf, 0},
						Span: []int32{15, 2, 4},
					},
				},
			},
		...
		model := api.NewTestAPI(nil, nil, nil)
		extractDefinitionLocations(model, fileDesc)
		wantLoc := api.SourceLocation{Filename: "oneof_test.proto", Line: 16}
		gotLoc, ok := model.DefinitionLocation(".test.oneof.Msg.choice")
		if !ok {
			t.Fatal("missing definition location for .test.oneof.Msg.choice")
		}
		if diff := cmp.Diff(wantLoc, gotLoc); diff != "" {
			t.Errorf("mismatch (-want +got):\n%s", diff)
		}
