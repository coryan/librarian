...
func TestGenerateOneOf(t *testing.T) {
	...
	oneofField1.Documentation = "A string field that is part of the oneof."

	oneofField2 := api.NewTestField("message_field").
		WithMessageType(inner)
	oneofField2.Documentation = "A message field that is part of the oneof."

	oneof := api.NewTestOneOf("choice").
		WithFields(oneofField1, oneofField2)
	oneof.Documentation = "A group of fields where only one is set."

	regField1 := api.NewTestField("regular_int32").
		WithType(api.TypezInt32)
	regField1.Documentation = "A regular field."

	regField2 := api.NewTestField("regular_string").
		WithType(api.TypezString)
	regField2.Documentation = "Another regular field."
	regField2.JSONName = "regularStringSpecial"
...
func TestGenerateOneOfWithKeyword(t *testing.T) {
	...
	headerField := api.NewTestField("header").
		WithType(api.TypezString)
	headerField.Documentation = "Specifies HTTP header name to extract JWT token."

	queryField := api.NewTestField("query").
		WithType(api.TypezString)
	queryField.Documentation = "Specifies URL query parameter name to extract JWT token."

	cookieField := api.NewTestField("cookie").
		WithType(api.TypezString)
	cookieField.Documentation = "Specifies cookie name to extract JWT token."
