...
func TestAnnotateTransport_Basic(t *testing.T) {
	...
	model := api.NewTestAPI([]*api.Message{reqMsg, respMsg}, nil, []*api.Service{svc}).
		...
		WithDefinitionLocation(respMsg.ID, "foo_service.proto", 10).
		WithDefinitionLocation(svc.ID, "foo_service.proto", 20)
	...
	if !ok || sAnn == nil {
		t.Fatal("svc.Codec is not *ServiceAnnotations")
	}
...
func TestAnnotateTransport_MixinsAndLRO(t *testing.T) {
	...
	model := api.NewTestAPI([]*api.Message{reqMsg}, nil, []*api.Service{svc}).
		WithDefinitionLocation(reqMsg.ID, "bar.proto", 1).
		WithDefinitionLocation(svc.ID, "bar.proto", 10)
...
func TestParseFloatDuration(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if gotSec != test.wantSec {
				t.Errorf("parseFloatDuration(%q) = %v, want %v", test.duration, gotSec, test.wantSec)
			}
		})
	}
}
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			if _, err := parseFloatDuration(test.duration); err == nil {
				t.Errorf("parseFloatDuration(%q) expected error, got nil", test.duration)
			}
		})
	}
}
...
func TestFormatFloat(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			if got := formatFloat(test.val); got != test.wantFmt {
				t.Errorf("formatFloat(%v) = %q, want %q", test.val, got, test.wantFmt)
			}
		})
	}
}
...
func TestBuildWrappedMethod_WithGRPCServiceConfig(t *testing.T) {
	...
	if err := os.WriteFile(cfgFile, []byte(jsonConfig), 0644); err != nil {
		t.Fatal(err)
	}
...
`,
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.yamlBody != "" {
				...
				if err := os.WriteFile(configFile, []byte(test.yamlBody), 0644); err != nil {
					t.Fatal(err)
				}
			}
...
`,
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if got != test.want {
				t.Errorf("isRestAsyncIOEnabled() = %v, want %v", got, test.want)
			}
		})
	}
}
...
func TestAnnotateTransport_Error(t *testing.T) {
	for _, test := range []struct {
	...
	}{
		{
			...
			wantErr:  "loading service config for MyService",
		},
		{
			name:     "malformed_grpc_service_config_json",
			filename: "myapi_grpc_service_config.json",
			content:  "{invalid_json",
			wantErr:  "loading gRPC service config for MyService",
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			tempDir := t.TempDir()
