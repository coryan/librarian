...
| :--- | :--- | :--- |
| **Equivalence Standard** | Exact byte-for-byte parity | Retain exact banner (`// Generated by the Codegen C++ plugin.`) and copyright header. Format emitted files via system `clang-format` so `diff -u` against reference files produces zero differences. |
...
- **Layers**:
  ...
  - **Layer 3.7: Formatting & Parity**: Run `clang-format` on emitted files and assert zero diff against golden files for pure gRPC services.
